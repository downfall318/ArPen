...
The lab includes the tiled carrier and an editable tile-HP list. It uses a repeatable random sequence shared across velocity bins so appending a graph preserves earlier tile choices. The game samples randomly per hit. Lab stopped-hit H/S represents global damage after the existing transfers: torso transfers calculated health and shock independently at 1×/1×; head transfers local health at 2× and applies calculated shock directly. Head shock includes its 3× zone factor exactly once in the stopped-hit formula; penetrating shock retains the incoming engine zone result without an extra multiplier. Existing local health calculations and health transfers are unchanged. Blunt health and shock profile multipliers each control their own channel. Penetrating wearer damage depends on the incoming DayZ damage result and is not numerically predicted by the lab.
...
This branch includes the F5 test panel, F6 damage HUD, and offline range mission. See [DEV_TESTING.md](DEV_TESTING.md) for setup. Production armor calculations and lab match `main`; test telemetry also reports sacrificed tiles and their HP loss.
